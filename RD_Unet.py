...
class RdUnet(nn.Module):
    def __init__(self, num_classes, weights=None):
        ...
        if weights:
            rdnet = timm.create_model('rdnet_tiny.nv_in1k', pretrained=False)
        else:
            rdnet = timm.create_model('rdnet_tiny.nv_in1k', pretrained=True)

        self.stem = rdnet.stem
        self.seq1 = rdnet.dense_stages[0]
        self.seq2 = rdnet.dense_stages[1]
        self.seq3 = rdnet.dense_stages[2]
        self.seq4 = rdnet.dense_stages[3]
        self.seq5 = rdnet.dense_stages[4]
        self.seq6 = rdnet.dense_stages[5]

        self.up5 = UpSampling(1464, 680)
        self.up4 = UpSampling(1360, 600)
        self.up3 = UpSampling(1200, 440)
        self.up2 = UpSampling(880, 256)
        self.up1 = UpSampling(512, 64)
        ...
        self.res = nn.Sequential(
            ...
            nn.BatchNorm2d(32),
            nn.Conv2d(32, num_classes, kernel_size=1, stride=1, bias=False),
        )
    ...
    def forward(self, x):
        ...
        x = self.up5(x, skip5, 1) #해상도 유지
        x = self.up4(x, skip4, 1) #해상도 유지
        x = self.up3(x, skip3, 2) #해상도 *2
        x = self.up2(x, skip2, 2) #해상도 *2
        x = self.up1(x, skip1, 1) #해상도 유지
        x = self.up0(x, skip0, 3) #해상도 *4
...
class UpSampling(nn.Module):
    def __init__(self, input_channels, output_channels):
        ...
        self.sequence1 = nn.Sequential(
            nn.ConvTranspose2d(input_channels, output_channels * 2, kernel_size=1, stride=1),
            nn.BatchNorm2d(output_channels * 2),
            nn.ReLU(inplace=True),
            nn.Conv2d(output_channels * 2, output_channels, kernel_size=1, stride=1, bias=False)
        )

        self.sequence2 = nn.Sequential(
            nn.ConvTranspose2d(input_channels, output_channels * 2, kernel_size=2, stride=2),
            nn.BatchNorm2d(output_channels * 2),
            nn.ReLU(inplace=True),
            nn.Conv2d(output_channels * 2, output_channels, kernel_size=1, stride=1, bias=False)
        )
        ...
        self.sequence3 = nn.Sequential(
            nn.ConvTranspose2d(input_channels, output_channels * 2, kernel_size=4, stride=4),
            nn.BatchNorm2d(output_channels * 2),
            nn.ReLU(inplace=True),
            nn.Conv2d(output_channels * 2, output_channels, kernel_size=1, stride=1, bias=False)
        )

    def forward(self, x, skip_connection, up):
...
save_dir = "C:/picture/ip_dataset/RD_UNET_MODEL"
...
if __name__ == "__main__":
    ...
    if weights:
        model = RdUnet(num_classes=30, weights=False)
        model.load_state_dict(torch.load(save_dir + '/model.pth', weights_only=True))
    else:
        model = RdUnet(num_classes=30, weights=True)
    model.to(device)
